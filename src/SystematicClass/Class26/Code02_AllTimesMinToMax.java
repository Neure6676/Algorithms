...
/**
 * 给定一个只包含正数的数组arr，arr中任何一个子数组sub（一定要连续），
 * 一定都可以算出(sub累加和 )* (sub中的最小值)是什么，
 * 那么所有子数组中，这个值最大是多少？
 *
 * 思路：以x做最小值的所有sub 将问题转化为求sub累加和最大
 * 使用单调栈找左右两边最近比x小的 中间的sub累加和是最大
 * 前缀和数组求累加和 O(1) [3,2,1,2,4,5] -> [3.5.6.8.12.17]
 */
public class Code02_AllTimesMinToMax {
    ...
    public static int max2(int[] arr) {
        ...
        int max = Integer.MIN_VALUE;
        Stack<Integer> stack = new Stack<>(); //栈中不放链表：相同的数最右侧的那个能够得到正确答案 不影响最终结果
