...
public class Code04_LongestCommonSubsequence {
    ...
    public static int process1(char[] str1, char[] str2, int i, int j) {
        if (i == 0 && j == 0) {
            ...
            // 看函数开始之前的注释部分
            // p1就是可能性c) 最长公共子序列，一定不以str1[i]字符结尾、但是可能以str2[j]字符结尾
            int p1 = process1(str1, str2, i - 1, j);
            // p2就是可能性b) 最长公共子序列，可能以str1[i]字符结尾、但是一定不以str2[j]字符结尾
            int p2 = process1(str1, str2, i, j - 1);
            // p3就是可能性d)最长公共子序列，必须以str1[i]字符结尾、也必须以str2[j]字符结尾，如果可能性d)存在，即str1[i] == str2[j]，那么p3就求出来，参与pk
