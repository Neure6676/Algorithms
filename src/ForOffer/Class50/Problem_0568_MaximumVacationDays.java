...
public class Problem_0568_MaximumVacationDays {
    ...
    public int maxVacationDays0(int[][] flights, int[][] days) {
        int nCity = days.length;
        int nWeek = days[0].length;
        int last = 0;
        int ans = 0;
        for (int i = 0; i < nWeek; i++) {
            int max = 0;
            for (int j = 0; j < nCity; j++) {
                if (days[j][i] > max && (last == j || flights[last][j] != 0)) {
                    max = days[j][i];
                    last = j;
                }
            }
    ...
    // dp[i][j]表示第i周一定在第j号城过 最多休息几天
    // 答案是最后一行求max
    public static int maxVacationDays1(int[][] flights, int[][] days) {
        int nCity = days.length;
        int nWeek = days[0].length;
        int[][] dp = new int[nWeek][nCity];
        // 第0周(行)
        ...
        for (int i = 1; i < nWeek; i++) {
            for (int j = 0; j < nCity; j++) {
                ...
                for (int city = 0; city < nCity; city++) {
                    if ((flights[city][j] != 0 || city == j) && dp[i - 1][city] != 0) {
                        int cur = dp[i - 1][city] + days[j][i];
    ...
    public static void main(String[] args) {
        ...
        System.out.println(maxVacationDays1(flights, days));
    }



    public static int maxVacationDays(int[][] fly, int[][] day) {
        int n = fly.length; // n cities
        int k = day[0].length; // k weeks
        // 哪些城能到它
        // pas[i] = {a, b, c}
        // 从a、b、c能飞到i
        int[][] pass = new int[n][];
