...
public class Code02_LeftRightSameTreeNumber {
    ...
    // 时间复杂度O(N * logN) master公式：T[N] = 2T[N/2] + O(N)
    public int sameNumber1(Node head) {
        if (head == null) {
            return 0;
        }
        int leftAns = sameNumber1(head.left);
        int rightAns = sameNumber1(head.right);
        return leftAns + rightAns + (same(head.left, head.right) ? 1 : 0);
    }

    private boolean same(Node h1, Node h2) {
        if (h1 == null ^ h2 == null) {
            return false;
        }
        if (h1 == null && h2 == null) {
